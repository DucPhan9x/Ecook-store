...
const chargeOrderSuccess = (req, res, next) => {
  ...
  try {
    ...
    // check timeOpen
    const timeOpenItem = _.find(timeOpen, {
    ...
    const start = parseFloat(
      timeOpenItem.openTime.getHours() +
        "." +
        timeOpenItem.openTime.getMinutes()
    );
    const end = parseFloat(
      timeOpenItem.closeTime.getHours() +
        "." +
        timeOpenItem.closeTime.getMinutes()
    );
    ...
    let discount = 0;
    if (promoCode) {
      discount = await orderService.calculateDiscount(
        vendorId,
        buyer._id,
        promoCode,
        subTotal
...
const chargeOrderCancel = (req, res, next) => {
  return res.render(`${this.rootModule}cancel`);
};

const createChargeOrder = (req, res, next) => {
  global.logger.info(
    "PayPalController::chargeOrder" + JSON.stringify(req.body)
  );
  ...
  try {
    ...
    // calculate shipping fee
    let shipping = await orderService.calculateShippingCost(
      req.user.id,
      vendorId
    );
    // calculate discount
    ...
    const creat_payment_json = {
      ...
      redirect_urls: {
        return_url: `${req.protocol}://${req.get(
          "host"
        )}/api/v1/payment/charge-success?userId=${
          req.user.id
        }&promoCode=${promoCode}&vendorId=${vendorId}`,
        cancel_url: `${req.protocol}://${req.get(
          "host"
        )}/api/v1/payment/charge-cancel`,
      },
      transactions: [
        {
          item_list: {
  ...
  try {
    ...
    const creat_payment_json = {
      ...
      redirect_urls: {
        return_url: `${req.protocol}://${req.get(
          "host"
        )}/api/v1/payment/deposit-success?userId=${
          req.user.id
        }&type=${type}&amount=${amount}`,
        cancel_url: `${req.protocol}://${req.get(
          "host"
        )}/api/v1/payment/deposit-cancel`,
      },
      transactions: [
        {
          item_list: {
...
const depositSuccess = (req, res, next) => {
  try {
